 ...
 *   · `docs/progress.md` 一直写着 `CI-green`，而 GitHub Actions 自 workflow 落地起**一次都没绿过**
 *     （2026-09-24 查实：15 次 run 全 failure，根因是 12 个测试文件硬编码了开发机绝对路径）。
 *     这是最坏的一类：**文档声称的事实从未成立过**，且本地跑什么都看不出来 ——
 *     「本地 `python ci.py` 全绿」推不出「CI 绿」。
 * 所以这里把「能机械核对的部分」钉死。判不了的部分（比如「N 项断言」的确切数字）不在此列 ——
 * 那要靠跑门禁，不靠读文档，因此**验收清单里索性不再写死断言总数**，改成指向 `ci.py` 的输出。
 *
 * ⚠️ **写这段黑名单时注意**：它判的是「这个字符串在不在文件里」，**不区分「宣称」与「引述」**。
 * 所以**更正一段旧说法时，不能把旧说法原样写出来**（哪怕只是为了说明"这句话不许再出现"）——
 * 写出来就会自己把自己判红。这个坑已经踩过两次：
 *   · `_test_assembly.js` 的注释里出现了 `window.top.postMessage`（判定「不许用」，注释却也中招）；
 *   · `docs/progress.md` 在列出"新拉黑了哪两个词"时把词照抄了一遍。
 * 正确写法是**用描述代替引用**（「那句 CI 全绿的说法」而不是把原话贴上去）。
 */
'use strict';
const fs = require('fs');
   ...
   删除 / 推翻的提案。两种情况都不该靠人的记性守着。 */
{
  const STALE = [
    ...
    ['docs/progress.md', 'CI-green',
      'CI 从未绿过（2026-09-24 查实：15 次 run 全 failure）—— 本地 `python ci.py` 绿 ≠ CI 绿'],
    ['README.md', '逐月增量没有单独记录',
      'v7 起每条规则带 hitDays 按天分桶，可算**精确**本月增量；老数据退回近似并明确标注'],
  ];
