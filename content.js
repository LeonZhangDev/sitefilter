...
(function () {
  ...
  /* ---------------- 站点模板：表与派生已抽到 site-templates.js ----------------
   * 站点表 + 它的派生（默认监管站点 / 候选选择器 / 设置页模板 / 番号直达 / 镜像组 /
   * 全局维度规则 / 维度作用范围）现在只有一份实现，content / background / options
   * 三端共用 —— 过去「加一个站要改 4 个地方，漏一处就静默失效」。
   * 本文件只把命名空间接出来，下文所有调用点名一律不变。
   * 「按 host 取模板 / 缓存 / 裸域兜底」依赖 location，必须留在本文件，见下。 */
  var SFS = (typeof globalThis !== 'undefined' && globalThis.SF_SITES)
    || (typeof window !== 'undefined' && window.SF_SITES) || null;
  if (!SFS) {
    // 显式失败，不静默降级：少了它站点识别整层失效，只会表现成「面板能开但没卡片」。
    throw new Error('SF_SITES 未加载：manifest 的 content_scripts 必须先加载 site-templates.js');
  }
  var SITE_TEMPLATES = SFS.SITE_TEMPLATES, LINK_KINDS = SFS.LINK_KINDS;
  var DEFAULT_SITES = SFS.DEFAULT_SITES, KNOWN_SELECTORS = SFS.KNOWN_SELECTORS,
      SELECTOR_TEMPLATES = SFS.SELECTOR_TEMPLATES;
  ...
  // 不同维度的默认作用范围（与设置页共用同一份，见 site-templates.js）
  var SCOPE_OF = SFS.SCOPE_OF;

  var COLORS = ['#00e5ff', '#ff4d6d', '#7c5cff', '#22c55e', '#ff9f1c', '#ffc93c'];

  /* ---------------- 下载链接探测 ---------------- */
  /* ---------------- 下载链接探测：解析层已抽到 magnet-core.js ----------------
   * 链接识别/解析/格式化（磁力字段、反混淆、体积格式化、归并排序）全是纯函数，
   * 已搬到 magnet-core.js —— 与设置页等其它入口共用同一份实现，杜绝两份解析器漂移。
   * 这里只把命名空间接出来，保持下文的调用点名不变。
   * 扫 DOM、装配结果集、页面打标记（probeLinks）仍在本文件。 */
  var SFM = (typeof globalThis !== 'undefined' && globalThis.SF_MAGNET)
    || (typeof window !== 'undefined' && window.SF_MAGNET) || null;
  if (!SFM) {
    // 显式失败，不静默降级：少了它「磁力一条都探测不到」会变成一个查不出来的谜。
    throw new Error('SF_MAGNET 未加载：manifest 的 content_scripts 必须先加载 magnet-core.js');
  }
  var PAN_HOSTS = SFM.PAN_HOSTS, URL_STOP = SFM.URL_STOP,
      MAGNET_RE = SFM.MAGNET_RE, ED2K_RE = SFM.ED2K_RE, THUNDER_RE = SFM.THUNDER_RE,
      TORRENT_RE = SFM.TORRENT_RE, MAGNET_PCT_RE = SFM.MAGNET_PCT_RE, B64_TOKEN_RE = SFM.B64_TOKEN_RE,
      SIZE_RE = SFM.SIZE_RE, QUALITY_RE = SFM.QUALITY_RE,
      MAGNET_HASH_RE = SFM.MAGNET_HASH_RE, MAGNET_SIZE_KEYS = SFM.MAGNET_SIZE_KEYS;
  ...
  /* 番号多站直达：纯本地拼 URL，零网络请求。站点表在 site-templates.js */
  var CODE_SITES = SFS.CODE_SITES;
  ...
  /* 镜像站点归一：分组表在 site-templates.js（与 SITE_TEMPLATES 同源派生） */
  var MIRROR_GROUPS = SFS.MIRROR_GROUPS;
  ...
  /* ---------------- 下载链接探测 ---------------- */
  /* hostOf() / panOf() 在 magnet-core.js（纯 URL 解析） */
  ...
  /* 磁力全字段解析（L2）在 magnet-core.js：parseMagnet() / decodeParam() */

  /* 磁力反混淆（L3）在 magnet-core.js：decodeObfuscated() / b64decode() / cleanupDecoded() */

  /* fmtBytes() / guessFromName() 在 magnet-core.js */
  ...
  /* mergeMagnet()（L4：同 infohash 归并）在 magnet-core.js */

  /* buildMagnetRaw() / magnetRank() / sortLinks()（L4：分层排序）在 magnet-core.js */
