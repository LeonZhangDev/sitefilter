...
四条都是纯本地、不涉及注册 / 进程，失败面小。H1 与 H3 建议不要跳 —— 它们改的是**已经会碰**
的交互（键盘、面板顶部开关），一旦回归是"用着别扭"而不是"一眼就崩"。

### H1 · 破坏性操作连按两下（防误触）

1. 打开面板、把鼠标悬停到某张卡片上（让 `j` / `k` 有作用对象）。
2. 按一下 `b`（默认屏蔽键）。
3. **预期**：卡片**没有**立刻消失，悬浮球闪一行「**再按 B 确认**」（或类似提示）。
4. **2.5 秒内**再按一次 `b`。
5. **预期**：这次才真的建屏蔽规则、卡片被隐藏 / 灰化（按当前屏蔽显示档）。

> ✘ 的判据：① 按一下就直接屏蔽了（防误触没生效）；② 按两下也屏蔽不了（提示后状态没接上）。
> 另：`f` 收藏 / `h` 高亮 / `p` 待看**不该**触发这一步（它们可逆）。
> 关掉开关复验：设置页「通用设置 → 行为开关」取消「破坏性操作连按两下确认」后，
> 按一下 `b` 应当**直接屏蔽**。

### H2 · 「🔍 诊断」页签

1. 打开一个有屏蔽命中（或"只看收藏 / 只看★番号"筛掉卡片）的监管站点列表页。
2. 面板 → **🔍 诊断** 页签。
3. **预期**：每张被处理过的卡片一行，列出**动作 / 维度 / 命中值**；**被隐藏的排最前**。
4. 点其中一行（挑一张**被隐藏的**）。
5. **预期**：页面滚到那张卡片、且该卡片**临时被揭示**（否则滚过去是一片空白），内容对得上。

> ✘ 的判据：① 诊断页空白（说明 `whyMap` 没被读出来）；② 点行滚过去**一片空白**（被隐藏的
> 没先揭示）；③ 诊断页里的条数与面板底部「👁 显示被隐藏（N）」的 N 明显不符。
> 它读的是悬停浮层同一份数据、**不重算规则** —— 所以结论不可能和"悬停查原因"不一致。

### H3 · 「⏸ 本页暂停」（会话级临时停用）

1. 在监管站点列表页，点面板顶部的 **⏸ 本页暂停**（或按 `Alt + X`）。
2. **预期**：扩展做过的事**全部撤回**，页面回到"像没装扩展"的样子（被隐藏的卡片回来、
   卡片上的标记消失）；**悬浮球不消失**，变成灰色的 ⏸。
3. 再点一下悬浮球（或再按一次 `Alt + X`）。
4. **预期**：**立刻恢复**到暂停前的样子。
5. 再验"刷新即恢复"：暂停 → 刷新页面 → **预期自动回到正常工作**（不残留停用态）。
6. 再验"不写设置"：暂停后再打开设置页，**行为开关等设置项一个都没变**。

> ✘ 的两个典型症状：① 悬浮球**一起消失**（那就没有恢复入口了，是最坏的一种）；
> ② 刷新后**仍然是停用态**（说明写进了 storage，成了"忘了关"的残留）。

### H4 · 隐藏来源标记（DevTools 确认，架构预防）

1. 在一个有卡片被屏蔽的列表页，打开 DevTools（`F12`）→ Elements。
2. 选一张**被屏蔽 / 被筛掉**的卡片。
3. **预期**：该元素带 `data-cf-hide-src` 属性，值为 `block`（命中屏蔽规则）或
   `filter`（被"只看收藏 / 只看★番号"筛掉）。
4. 把那张卡片的屏蔽规则**改成收藏**（或反向）后，让面板重算一次。
5. **预期**：属性**跟着重算结果变**（清掉或改成新值），不留旧值残影。

> 这条是**架构预防**而非用户功能：它保证"将来再加一个会隐藏卡片的模块"时能直接归因，
> 不必面对"谁都认为自己没隐藏、但卡片就是不见"。属性**不参与任何 CSS 选择器**，
> 所以看到它时页面的**可见行为应当和没有它时完全一样**。
> 注：本轮新增「弃」之后，该属性的取值多了第三种 `drop`（见 I3）。

**I0 · 先把 H4 补一个新取值**：把某张卡标成「弃」（见 I4）后，该卡片元素的
`data-cf-hide-src` 应当是 **`drop`**，而不是 `block` —— 诊断页靠它区分"规则挡的"和"我弃的"。

---

## I. 本轮新增能力（可选走查）

四条都不碰注册 / 进程，全部纯本地。**I1（例外）建议不要跳**：它改的是"卡片到底显不显示"这条
最核心的判断链，一旦回归是**功能不对**而不是"用着别扭"。

### I1 · 规则级例外：放行（`allow`）

**先造一个"误杀"**：
1. 在某位**有不止一部作品**的女优卡片上右键 →「屏蔽「某某」」建一条屏蔽规则。
2. 刷新 / 让面板重算，**预期**：她的卡片全部消失（按当前屏蔽显示档）。

**现在用例外把它摘出来**：
3. 设置页 → 规则管理 → 新建规则：类型选 **番号**，值填她那部**你想留的**番号（如 `ABC-001`），
   动作选 **放行（例外）**，保存。
4. 回到列表页重算。**预期**：**只有那一个番号**的卡片回来了，她的其他作品**仍然是挡着的**。

> ✘ 的判据：① 例外写了但一张都没回来（例外没接进判定链）；② **她的全部作品都回来了**
> （例外被当成了"有例外就全放行"——这是最容易写错的一种，也是 I1 里唯一必须核的一条）。
>
> 顺带三项（可跳过、出问题也不影响主流程）：
> - **例外不吃排序**：打开设置页「首条命中生效」，把那条例外**拖到屏蔽规则下面**，回页面重算 ——
>   例外**仍然生效**（逃生门排第几位都该生效；如果它因为排后面而失效，就是回归）。
> - **例外不阻断正向动作**：再给那位女优建一条**收藏**规则，回页面 —— 命中例外的那个番号
>   应当**可见且带收藏标记**。
> - **例外不会被误报成死规则**：设置页 →「规则体检」，那条例外**不应**出现在"从未命中"里。

### I2 · 影片级评分 / 备注

1. 在任意卡片上右键 → 点第 **5** 颗星。
2. **预期**：悬浮球闪「已评 5★」；面板 **★ 番号** 页里该行显示 5 颗实心星。
3. 右键 →「写备注」→ 输入一段字 → 确定。
4. **预期**：★号 页该行标题下出现 **📝 备注**；重开面板仍在（说明落盘了）。
5. 右键 → 点 **`✕` 清除评分** → **预期**：星变空，**但备注还在**（两件事各存各的）。
6. 再清一次备注（留空）→ **预期**：该番号的标记整个消失。

> ✘ 的判据：① 打完分重开面板就没了（没落盘）；② 清评分把备注一起清掉了。
> 备注是给自己看的，**不参与任何计算**；评分才会经参演者折算成推荐信号，所以**别拿评分当备注用**。

### I3 · 规则包来源可追溯

1. 设置页 → 规则管理 → 规则包模板 → 选任意一个包 → 导入。
2. **预期**：新导入的规则在规则列表里带一行 **📦 包名 v版本** 的脚注，悬停能看到导入时间。
3. **预期**：**手动输入的**老规则**没有**这个脚注（没来源 ≠ 来源未知）。

> 这条是给你自己排查用的：以后某个包改坏了，能一眼看出"我这条是哪版导进去的"。
> 所以**改动包内容时要顺手抬 `version`** —— 不抬，这个脚注就是假的。

### I4 · 「弃」（番号级否定）

1. 在任意卡片上右键 →「弃」（或面板 ★番号 页每行的「弃」按钮）。
2. **预期**：该卡片**立刻隐藏**；★号 页那一行变成 **⛔ + 「弃」角标**、整行降到 62% 透明度。
3. DevTools 看一眼该卡片元素：`data-cf-hide-src` = **`drop`**（不是 `block`）。
4. 打开 **软屏蔽**（面板顶部开关），让页面重算。
5. **预期**：被弃的卡片**还是不显示**，**不会**浮出「仍然查看」按钮。
6. 右键再点一次「弃」（或 ★号 页点「取消弃」）→ **预期**：卡片恢复可见，角标消失。

> ✘ 的判据：① 点了弃卡片还在（没接进判定）；② **软屏蔽下被弃的卡浮出了「仍然查看」**
> —— 那是给屏蔽规则留的后门，弃**没有后门**（如果能点开，就说明这两条判定串了）。
> 另：弃**不进规则库**，所以设置页的规则条数**不该**因为弃而变化。
