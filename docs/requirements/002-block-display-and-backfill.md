...
状态：**已实施**（L1 三档屏蔽 + L2 番号站补足均已落地；下面立项阶段的讨论原文保留不回改）
提出人：Leon
记录时间：2026-09-22
相关文件：`content.js` / `content.css` / `options.js` / `options.html` / `popup.html` / `README.md`

> **实施状态（2026-09-23 补）** —— 本文写于立项讨论阶段，下面的「待 Leon 确认」「若 Leon 同意」
> 等措辞按原样保留（历史锚定）。实际落地情况：
> - **L1 三档**：`settings.blockDisplay = 'hide' | 'placeholder'（默认）| 'soft'`（灰化遮罩）。
>   旧的二元开关 `softBlock` 在 `SCHEMA_VERSION` v5→v6 迁移里按原值映射（`true`→`soft`、
>   `false`→`hide`）后删除，避免两份真相并存。
> - **L2 补足**：只有 `JavDB580` 一个站可行（判定依据见
>   [004](004-pagination-and-magnet-site-verification.md)），故放行名单收口为站点模板上的
>   `bf: true`，**全库仅此一处**，且开关默认关闭（全库唯一会发网络请求的功能）。
> - **翻页失效的护栏**：加了卡片指纹去重，翻页拿不到新内容时降级为「什么都不补」，
>   而不是把重复卡片贴上去。
